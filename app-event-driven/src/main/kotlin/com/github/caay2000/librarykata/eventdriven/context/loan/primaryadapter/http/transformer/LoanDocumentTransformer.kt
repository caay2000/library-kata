...
class LoanDocumentTransformer(val resourceBus: ResourceQueryBus) : Transformer<Loan, JsonApiDocument<LoanResource>> {
        ...
        if (include.contains("account")) {
            val loans =
                resource.findAllRelationshipWithType(AccountResource.TYPE).map {
                    resourceBus.invoke(ResourceQuery(it.id, AccountResource.TYPE)).resource.toJsonApiIncludedResource()
                }
        ...
        if (include.contains("book")) {
            val loans =
                resource.findAllRelationshipWithType(BookResource.TYPE).map {
                    resourceBus.invoke(ResourceQuery(it.id, BookResource.TYPE)).resource.toJsonApiIncludedResource()
                }
            includes.addAll(loans)
        }
